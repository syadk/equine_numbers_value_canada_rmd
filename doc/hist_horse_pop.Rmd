...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
kable(max_horses, 
      caption = "Table 1. Max and min number of hourses per province for years 1940 - 1972")
```

Below we zoom in and look at the province of Alberta:

```{r plot province, fig.width=5, fig.height=4, fig.cap = "Number of Horses in each province over time", out.width = '100%'}
